const tableData = `| ProductID | ProductName                      | SupplierID | CategoryID | Unit                 | Price  |
| :-------- | :------------------------------- | :--------- | :--------- | :------------------- | :----- |
...
rows.forEach((row) => {
  ...
  const categoryId = parseInt(rowData[3]);
  const unit = rowData[4];
  const price = parseFloat(rowData[5]);

  // const insertStatement = `INSERT INTO products (ProductID, ProductName, SupplierID, CategoryID, Unit, Price) VALUES (${productId}, '${productName}', ${supplierId}, ${categoryId}, '${unit}', ${price});`;
  const insertStatement = ` (${productId}, "${productName}", ${supplierId}, ${categoryId}, "${unit}", ${price})`;
  insertStatements.push(insertStatement);
});

let resSQL = `INSERT INTO products (ProductID, ProductName, SupplierID, CategoryID, Unit, Price) VALUES`;
